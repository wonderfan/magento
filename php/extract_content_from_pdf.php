...
class IndexController extends Zend_Controller_Action
{
    ...
    {
        $file = dirname(__FILE__).DIRECTORY_SEPARATOR.'zend.pdf';        
        $pdf = Zend_Pdf::load($file);  
        $content = $this->pdf2txt($pdf->render());    
        $matches = array();
        preg_match_all('/(zend|Framework)/i', $content, $matches);
        $matchedContent = '';
        foreach ($matches as $value){           
            $matchedContent.=implode('</br>' ,$value);
        }     
        $this->view->content = $matchedContent;
        $this->view->mytest = $file;
    }
